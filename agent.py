...
def turns_to_wait(player, research_points_target, debug=False):
  if player.research_points >= research_points_target:
    return 0
  for t in range(player.turn, MAX_DAYS):
    r = player.RP_ESTIMATOR(t)
    if debug:
      prt(f'[predicted] t={t}, rp={r}')
    if r >= research_points_target:
      return t - player.turn
  return MAX_DAYS
...
class LandingInfo:
  ...
  def __str__(self):
    return f"Landing(u={self.oppo_unit.id}@{self.oppo_unit.pos}, d={self.dist}, landing={self.is_landing})"
...
class ClusterInfo:
  ...
  def cell_next_to_target_cluster(self, worker, near_resource_tile, use_nb9=True,
                                  debug=False):
    ...
    cluster_ids = self.get_neighbour_cells_cluster_ids(near_resource_tile.pos,
                                                       use_nb9=use_nb9)
    # if debug:
      # prt(f' >> {worker.id}, cell={near_resource_tile.pos} target_cluster_id={worker.target_cluster_id}'
...
class OffenderDetector:
  ...
  def get_cluster_dist(self, cluster, unit, debug=False, add_threaten_point=False):
    min_positions = None
    min_dist = MAX_PATH_WEIGHT
    threaten_positions = []
    # if unit.id in DRAW_UNIT_LIST:
  ...
  def compute_oppo_unit_target_cluster(self, unit):
    ...
    1) unit to cluster dist: d0 <= ATTACK_CHECK_DIST
    2) last position dist: d1 > d0
    3) only attack on nearest landing points

    Threaten points:
    0) dist to cluster <= NEAR_CHECK_DIST
    """
    ...
    for cid in unit.shortest_path.reached_cids:
      ...
      min_dist, min_positions = self.get_cluster_dist(cluster, unit, debug=True,
                                                      add_threaten_point=True)
      ...
      for min_pos in min_positions:
        ...
        last_min_dist, last_positions = self.get_cluster_dist(cluster, last_unit)
        if last_positions is None:
          continue

        # if min_pos in MAP_POS_LIST:
          # prt(f' ## {min_pos} min_dist={min_dist}, last_min_dist={last_min_dist}, last_positions={last_positions}')

        # Approaching, at least not leaving
        if min_dist <= last_min_dist:
          if min_dist < landing_dist:
            landing_dist = min_dist
            landing_positions = [(min_dist, min_pos)]
          elif min_dist == landing_dist:
            landing_positions.append([min_dist, min_pos])

      landing_candidates.extend(landing_positions)

    if not unit_on_cluster:
      for dist, pos in landing_candidates:
        i = self.ci.add_landing_info(pos, unit, dist, is_landing=True)
        # prt(f' [L] {i}')
...
class Strategy:
  ...
  def update_unit_info(self):
    ...
    for unit in self.game.opponent.units:
      shortest_path = ShortestPath(self.game, unit, ci=self.cluster_info)
  ...
  def assign_worker_target(self, workers, plan_idx=0):
    ...
    def is_resource_tile_can_save_dying_worker(resource_tile,
      ...
      if (is_worker_on_last_city_tiles(worker, debug) or
          is_deficient_resource_tile(resource_tile)):
        if debug:
          prt(f' > not valid resource {resource_tile.pos}: defiiccient {is_deficient_resource_tile(resource_tile)}'
              f' worke_on_last_city={is_worker_on_last_city_tiles(worker)}')
        return False
    ...
    def is_worker_on_last_city_tiles(worker, debug=False):
      citytile = worker.cell.citytile
      if citytile is None:
        return False
      city = self.player.cities[citytile.cityid]
      city_last = not city_wont_last_at_nights(self.game.turn, city, debug=debug)
      return city_last
    ...
    def get_resource_weight(worker, resource_tile, arrival_turns, quick_path):
      ...
      save_dying_wt = 0
      if is_resource_tile_can_save_dying_worker(resource_tile, worker, debug):
        save_dying_wt += UNIT_SAVED_BY_RES_WEIGHT
      ...
      v = ((wt) / dd(arrival_turns) + boost_cluster + fuel_wt +
           opponent_weight + default_res_wt + wood_full_boost + save_dying_wt)
      if worker.id in DRAW_UNIT_LIST and resource_tile.pos in MAP_POS_LIST and plan_idx == 1:
        # prt(f"t={self.game.turn} w[{worker.id}] v={v}, res={resource_tile.pos}")
    ...
    def get_near_resource_tile_weight(worker, near_resource_tile, arrival_turns,
      ...
      def compute_build_turns(path, debug=False):
        ...
        if (is_worker_full_at_cur_cell or is_transfer_build_position):
          # if debug:
            # prt(f'[SKIP_NRT] woker_full={is_worker_full_at_cur_cell}'
                # ' is_transfer_build={transfer_build_wt}')
          return 0

        dest_state = path.get_dest_state(near_resource_tile.pos)
        if dest_state is None:
          if debug:
            prt(f'[SKIP_NRT] {near_resource_tile.pos} dest_state={dest_state}')
          return MAX_DAYS
        ...
        if is_connection_point:
          ...
          # TODO: remove one_step_amt condition?
        # Use one_step_amt to check resource researched, why not use wait_turns?
          if one_step_amt == 0 or wait_turns >= MAX_DAYS:
            if debug:
              prt(f'[SKIP_NRT] {near_resource_tile.pos} one_step_amt={one_step_amt}, wait_turns={wait_turns}')
            return MAX_DAYS
      ...
      is_next_to_target_cluster = self.ci.cell_next_to_target_cluster(
          worker, near_resource_tile, debug=debug)
      ...
      if is_defend_position:
        ...
        if is_worker_on_cluster:
          ...
          if is_landing_point:
            ...
            opponent_weight += 1000 / dd(arrival_turns, r=1.2)
            oppo_weight_type = 'threaten'

          if is_connection_point:
            # Only protect connection point at close dist
            if oppo_arrival_turns > 2:
              opponent_weight = 0
            # lower opponent_weight at connection???, WHY?
            # opponent_weight *= 0.6

            # Do not defend rule.
            if (near_resource_tile.n_citytile_neighbour >= 2
                or (self.circle_turn >= 27 and worker.surviving_turns <= 10)):
              opponent_weight = 0
      ...
      if worker.id in DRAW_UNIT_LIST and near_resource_tile.pos in MAP_POS_LIST and plan_idx == 1:
        prt(f"[defend] {near_resource_tile.pos} landing_point={is_landing_point}, "
            ...
            f" near_oppo_unit={nearest_oppo_unit.id}@{nearest_oppo_unit.pos}")
      ...
      if build_city_bonus:
        ...
        # if  or worker_has_coal_or_ruanium:
      ...
      if debug:
        prt(f'[NRT] t={self.game.turn} w[{worker.id}] nrt[{near_resource_tile.pos}], is_oppo_city={int(is_opponent_citytile)} '
            ...
            f' next_to_target_cluster={is_next_to_target_cluster} save_dying_wt={save_dying_wt}'
           )
      return v
    ...
    def get_city_tile_neighbour_weight(worker, cell):
      ...
      if boost > 0:
        ...
        self.worker_build_city_tasks[(worker.id, cell.pos)] = self.quickest_path_pairs[worker.id][0]
      return wt
  ...
  def compute_worker_moves(self):
    ...
    def get_step_weight(worker, next_position, quick_path, next_step_positions):
      ...
      if worker.id in DRAW_UNIT_LIST and DRAW_UNIT_MOVE_VALUE:
        prt((f"[STEP] t={self.game.turn} w[{worker.id}]@{worker.pos}, next[{next_position}], v={v}, bg={bg_score}"
  ...
  def update_player_info(self):
    ...
    self.player.avg_research_point_growth = self.player.unit_count / CITY_ACTION_COOLDOWN
    self.player.turn = self.game.turn

    opponent = self.game.opponent
    opponent.avg_research_point_growth = opponent.unit_count / CITY_ACTION_COOLDOWN
    opponent.turn = self.game.turn
      ...
      for target_cell in next_cells:
        ...
        for nb_cell in nb_cells:
          ...
          is_nb_unit_can_collect_and_build = (
              neighbour_cells_can_collect_and_build_citytile(nb_cell, nb_unit, worker))
  ...
  def send_worker_to_target(self):
    ...
    def clear_idle_worker_tasks_dict(tasks, worker_ids):
      return {(uid, pos) : path for (uid, pos), path in tasks.items()
  ...
  PLAYER_RESARCH_POINTS = []
  OPPO_RESARCH_POINTS = []
  def record_research_points(self):
    turn = self.game.turn

    def add_points(target, v):
      if is_night(turn):
        return
      if target and v <= target[-1][1]:
        return
      target.append((turn, v))


    add_points(self.PLAYER_RESARCH_POINTS, self.game.player.research_points)
    add_points(self.OPPO_RESARCH_POINTS, self.game.opponent.research_points)

    def fit(history, player):
      def simple(t):
        add = (t - turn) * player.avg_research_point_growth
        return player.research_points + add

      if turn < 20:
        return simple

      turns, points = zip(*history)
      # turns = range(len(points))
      f = np.poly1d(np.polyfit(turns, points, 2, w=turns))
      return f


    self.game.player.RP_ESTIMATOR = fit(self.PLAYER_RESARCH_POINTS, self.game.player)
    self.game.opponent.RP_ESTIMATOR = fit(self.OPPO_RESARCH_POINTS, self.game.opponent)
