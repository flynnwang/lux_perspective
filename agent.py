...
class QuickestPath:
  ...
  def compute(self):
    ...
    def wait_then_move(cur_state, extra_wait_days=0, debug=False):
      ...
      cur_state = self.wait_for_cooldown(cur_state, extra_wait_days=extra_wait_days, debug=debug)
      ...
      for nb_cell in get_neighbour_positions(cur_state.pos, self.game_map,
        ...
        if not next_state:
          if debug:
            print(f' skip move: cur_pos={cur_state.pos}, turn={cur_state.turn} to {nb_cell.pos}')
          continue
    ...
    while q:
      ...
      debug = False
      if (self.worker.id in DRAW_UNIT_LIST and cur_state.cell.pos in MAP_POS_LIST
      ...
      if is_player_citytile and is_night(cur_state.turn):
        ...
        if debug:
          print(f' wait_then_move (2): cur_stat.turn={cur_state.turn},cd={cur_state.cooldown} extra_wait={extra_wait_days}, cooldown_wait_days={cooldown_wait_days}')
    ...
    while q:
      ...
      for prev_pos in st.prev_positions:
        ...
        if self.debug and prev.pos in [Position(5, 5)]:
          ...
          print(f' >> prev {prev.pos} prev.turn={prev.turn}, st{st.pos}, st_turn={st.turn}, st_extra={st.extra_wait_days}')
...
class ClusterInfo:
  ...
  def __init__(self, game):
    self.game = game
    self.game_map = game.game_map
    self.position_to_cid = np.ones((self.game_map.width, self.game_map.height),
                                   dtype=np.int64) * -1
...
class Strategy:
  ...
  def get_cluster_boundary_cell_positions(self, turn, cid):
    ...
    for x, y in zip(x_pos, y_pos):
      cluster_pos = Position(x, y)
      for nb_cell in get_neighbour_positions(cluster_pos, self.game_map, return_cell=True):
  ...
  def assign_worker_target(self, workers):
    ...
    def is_resource_tile_can_save_dying_worker(resource_tile, worker):
      ...
      if (is_worker_on_last_city_tiles(worker)
          or is_deficient_resource_tile(resource_tile)):
        return False

      # Can't reach this resource/near tile.
      quick_path, arrival_turns = self.select_quicker_path(worker, resource_tile.pos)
    ...
    def get_resource_weight(worker, resource_tile, arrival_turns, quick_path):
      ...
      if fuel_wt > 0:
        ...
        if (nearest_oppo_unit and min_turns <= 8
            and arrival_turns <= min_turns
            and cid in get_opponent_unit_nearest_cluster_ids(nearest_oppo_unit)
            and (self.get_min_cluster_arrival_turns_for_opponent_unit(cid, nearest_oppo_unit)
                 == min_turns)):
      ...
      if city_wont_last and n_citytile >= 2:
        ...
        if (arrival_turns_wo_city == min_city_arrival_turns    # only goto nearest city tiles.
          ...
          not_full_woker_goto_city = (
              ...
              and cargo_total_fuel(worker.cargo) >= 60
          )
    ...
    def get_near_resource_tile_weight(worker, near_resource_tile, arrival_turns, quick_path):
      ...
      surviving_turns = get_surviving_turns_at_cell(worker, quick_path, near_resource_tile)
      ...
      if (worker.is_cluster_owner and not is_next_to_target_cluster
          and cell_has_player_citytile_on_target_cluster(worker, near_resource_tile)):
        build_city_bonus = False
      ...
      if fuel_wt > 0:
        ...
        if (nearest_oppo_unit and min_turns <= 8
            and arrival_turns <= min_turns):
          unit_nearest_cluster_ids = get_opponent_unit_nearest_cluster_ids(nearest_oppo_unit)
          cell_cluster_ids = get_near_resource_tile_cluster_ids(near_resource_tile)
          cell_is_nearest_in_cluster = any((self.get_min_near_cluster_arrival_turns_for_opponent_unit(cid, nearest_oppo_unit)
                                            == min_turns) for cid in cell_cluster_ids)
          if (unit_nearest_cluster_ids & cell_cluster_ids
              and cell_is_nearest_in_cluster):
    ...
    for i, worker in enumerate(workers):
      for j, target in enumerate(target_cells):
        ...
        if cell_has_player_citytile(target, self.game):
          ...
          v = get_resource_weight(worker, target, quicker_dest_turns, quicker_path)
          ...
          v = get_near_resource_tile_weight(worker, target, quicker_dest_turns, quicker_path)
  ...
  def compute_worker_moves(self):
    ...
    def get_step_weight(worker, next_position, quick_path, next_step_positions):
      ...
      if (next_position == worker.target_pos
          or next_position in next_step_positions):
    ...
    for worker_idx, worker in enumerate(workers):
      ...
      for next_position in gen_next_positions(worker):
        wt = get_step_weight(worker, next_position, quick_path, next_step_positions)
  ...
  def worker_look_for_resource_transfer(self):
    ...
    def neighbour_worker_with_enough_resource(worker):
      ...
      for target_cell in next_cells:
        ...
        collect_amt = collect_amount_at_cell(target_cell, self.player, self.game_map)
        ...
        for nb_cell in nb_cells:
          ...
          is_nb_unit_can_move_and_build = (neighbour_cells_can_build_citytile(nb_cell, nb_unit)
          ...
          nb_amt, _ = get_one_step_collection_values(nb_cell, self.game.player, self.game, unit=nb_unit)
          if (nb_amt + cargo_total_amount(nb_unit.cargo) >= CITY_BUILD_COST
              and is_nb_unit_can_build):
            continue
